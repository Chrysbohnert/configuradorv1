...
router.get('/equipamentos', requireAuth, requireAdminFull, asyncHandler(async (req, res) => {
  const data = await svc.listarEquipamentosComCusto();
  return res_.ok(res, data, { count: data.length });
}));

router.post('/importar', requireAuth, requireAdminFull, asyncHandler(async (req, res) => {
